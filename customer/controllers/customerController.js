...
const kafka = new Kafka({ clientId: 'customer-service', brokers: ['44.214.218.139:9092', '44.214.213.141:9092'] });
/**
...
exports.addCustomer = async (req, res) => {
    ...
    if (!userId || !name || !phone || !address || !city || !zipcode || !state) {
        const message = 'missing some field';
        res.status(400).send({ message: message });
    } else if (!validator.isEmail(userId)) {
        const message = 'invalid email';
        res.status(400).send({ message: message });
    } else if (state.length != 2) {
        const message = 'state should be to 2 letter';
        res.status(400).send({ message: message });
    } else {
        primaryDb.query(insertQuery, [userId, name, phone, address, address2, city, state, zipcode], async (err, dbres) => {
            if (err) {
                if (err.code === 'ER_DUP_ENTRY') {
                    const message = 'This user ID already exists in the system.';
